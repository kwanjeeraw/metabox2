...
#'@param method name of normalization method. Choose one from the list: contrast, cubic, eigenms, linear, liwong, loess, mean, median, pqn, quantile, sum, vsn. Default is median.
...
#'@references Cheng Li, et al. liwong (2001) \url{https://doi.org/10.1186/gb-2001-2-8-research0032}.
...
normalize_input_data_bydata <- function(METBObj, method="median", ref=NULL){
  #Check argument
  tmparg <- try(method <- match.arg(method, c("contrast","cubic","eigenms","linear","liwong","loess","mean","median","pqn","quantile","sum","vsn"), several.ok = FALSE), silent = TRUE)
  if (class(tmparg) == "try-error") {
    cat("\nERROR! Argument 'method' is not valid, choose one from the list: contrast,cubic,eigenms,linear,liwong,loess,mean,median,pqn,quantile,sum,vsn.\nData was not normalized.\n")
    return(METBObj)
  }
  if(sum(is.na(METBObj$X)) > 0){#Data contains missing values
    cat("\nThe data contains missing values. contrast, cubic and liwong methods will not normalize the data. The others will retain NAs in the normalized data.
  ...
  if (method == "liwong"){#liwong
    ...
    if(length(unlist(inst_pkg))){
      ...
      new_dat = data.frame(t(liwong_data), row.names = c(1:nrow(dat)))
  ...
  METBObj$X = new_dat; METBObj$details$normalize_bydata = methodls; #update input data
  return(METBObj)
}
